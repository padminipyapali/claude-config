...
- **A displayed number and the money derived from it must reduce over ONE source-of-truth array.** When a UI renders a quantity (hours, items, units) next to a value computed from it (pay, price, total cost), both must aggregate over the same entry set. If the number sums set A while the amount sums set B, they silently disagree — and the user catches it by dividing (`$2,150 / 50h = $43/h` revealed the hours label dropped a paid-PTO day the pay kept). Sibling-sweep EVERY surface that shows the pair (e.g. Calendar / Hours tab / Dashboard / PDF) — one view drifting while the others are correct is the common shape. Fix by deriving the displayed number from the exact array the money calc consumes, not a parallel `entries.reduce(...)`. <!-- Source: bug fix, nanny-management #53, 2026-06-16 -->
- Inline editing "clear" guard: compare against previous value, not empty string, or clearing is blocked.
- **Cleared form inputs persist `''`, and `??` won't fall back on it.** A cleared `<input type="time">`/`type="number"`/`type="date"` fires onChange with `value === ''`. If you persist it raw and later resolve a default with `value ?? fallback`, the empty string is NOT null/undefined so `??` returns `''` — a blank field that flows downstream (e.g. `calculateHours('', ...)` → `parse` → NaN saved). Fix at BOTH layers: write side stores `e.target.value || undefined` (don't persist junk), and resolution uses `||` not `??` (empty string is never a valid time/number/date, so it should fall back). Defensive: never assume a controlled input's value is non-empty just because it has a default. <!-- Source: critic+adversarial review, nanny-management #51, 2026-06-16 -->
- **"Add form" open handlers that snapshot context-derived defaults in a `useState` initializer should re-read defaults on open.** A `useState(() => resolveDefaults(ctx))` initializer runs once at mount; if `ctx` (e.g. `selectedNanny`) loads async after first render, opening the form later shows stale defaults until a remount/reset. Re-resolve on the open handler (as a `useCallback` whose deps include the resolved defaults), mirroring how a click-to-open-with-args handler already does. <!-- Source: adversarial review, nanny-management #51, 2026-06-16 -->
...
- **expo-router: an auth/onboarding gate must be BIDIRECTIONAL.** If two group index routes both claim `/` (e.g. `app/(tabs)/index.tsx` AND `app/(onboarding)/index.tsx`), `/` resolves to one of them and `unstable_settings.initialRouteName` does NOT reliably disambiguate the match — so a cold start can land on the "wrong" group. A one-directional redirect (only `!authed → /(onboarding)`) then strands a COMPLETED user who lands on onboarding, with nothing sending them back to `/(tabs)`. Bug is invisible mid-session (you `router.replace("/(tabs)")` after completing) and only bites on relaunch. Fix: redirect BOTH ways off `useSegments()[0]` — `authed && inOnboarding → /(tabs)` as well as `!authed && !inOnboarding → /(onboarding)`. Defer `SplashScreen.hideAsync()` until `segments.length > 0` (resolved) AND no redirect pending, so the transient `/`→wrong-group mount never flashes. <!-- Source: baby-name-picker #140, 2026-05-31 -->
- **Device-symptom bugs: REPRODUCE + INSTRUMENT before theorizing a fix.** A "re-onboards on cold start" bug was first mis-fixed by hardening `initialize()` (PR #136, wrong — init was never the cause), then chased through three more wrong theories. Ground truth came from throwaway breadcrumb probes written to a SQLite table at each lifecycle stage (init stages; the redirect effect's `usePathname`/`useSegments`), cold-relaunched via `xcrun simctl terminate/launch`, and read back with `sqlite3` against the sim's user.db. The trace (`PATH / segments=["(onboarding)"]` while the flag was already `true`) located it instantly. Revert probes; verify the fix the same way (screenshot the cold start) before merging. <!-- Source: baby-name-picker #140, 2026-05-31 -->
- **A React error boundary is NOT a safety net for an async-driven screen.** Boundaries catch render/lifecycle throws ONLY — never errors from event handlers, promises, store actions (Zustand), timers, or animation callbacks. A screen whose content depends on an async load that throws on a non-render path goes **silently blank with no fallback** (the boundary never sees it). Every such screen needs its OWN terminal error state with a recovery action (a `loadError` flag → "couldn't load · Try again" branch), distinct from its empty/loading states. Symptom that fingerprints this class: **blank/spinner, no error message, survives force-restart** → the trigger is deterministic persisted data read on launch, not a transient glitch; hunt the disk-read input (e.g. a persisted filter/exclusion set starving a fixed candidate pool). Corollary: use **typed errors for control flow** — when one error means "expected, handle gracefully" (e.g. `PoolExhaustedError`) and any other means "unexpected, show recovery," branch on `instanceof`; never let a bare `throw` from a query ride the same channel as the expected one into a fire-and-forget `.catch` that only logs (the log goes nowhere without a crash reporter, so it surfaces only via a user screenshot). <!-- Source: baby-name-picker, fix/compare-blank-screen, 2026-06-04 -->
...
- **A fixed-height, `overflow: "hidden"`, vertically-centered (`justifyContent: "center"`) container has NO safe overflow direction.** When variable content exceeds the box, centered overflow spills off the top AND bottom equally and clips symmetrically — the *first* child (often the visual hero, e.g. a large title) loses its top, and any absolutely bottom-anchored element collides with the overflowing bottom. Don't rely on centering to "just fit" variable-length content. Fix by bounding content height (collapse/summarize the variable part — progressive disclosure) or by top-anchoring + scroll. Symptom looks like a "padding bug" but is really content-overflow. <!-- Source: baby-name-picker, compare-card progressive disclosure, 2026-05-28 -->
...
- **contenteditable text inputs need a four-guard cluster, applied together:** (1) `contenteditable="plaintext-only"` so pasted rich-text HTML can't render (or print); (2) suppress Enter (`keydown` preventDefault) for single-paragraph fields — otherwise nested `<div>`s make `textContent` silently join lines with no separator, diverging persisted data from what's on screen; (3) any serializer that reads the field must null-tolerate missing/empty nodes (`?.textContent ?? fallback ?? ""`); (4) treat a cleared field ("") as revert-to-default, or the empty override permanently strands the original content. All four were missed by the implementer and caught only by a fresh-context critic on plush-press #4. **Fifth guard when the text feeds a print/export pipeline: normalize typography at the print boundary.** contenteditable-sourced (or chat-relayed) text carries straight apostrophes (`'`), straight quotes, and three-dot ASCII ellipsis (`...`) into print output where typographic ones (`’` `“ ”` `…`) are expected — invisible on screen, glaring in a printed book. Normalize in the exporter/print layer, not as one-off content edits, so re-synced or re-pasted text can't reintroduce them (plush-press #7: critic round 1 caught straight apostrophes/ellipsis in 3 board-book spreads sourced from book.html's contenteditable captions). <!-- Source: post-mortem, plush-press #4, 2026-06-09; strengthened plush-press #7, 2026-06-09 -->
- **Guard every `localStorage` access with try/catch in pages meant to open from `file://` (or any blocked-storage context).** A throwing `getItem` inside a render loop aborts the whole loop — one storage failure blanked an entire 12-page render except the first item. Reads fall back to in-code defaults; writes become silent no-ops (feature still works for the session, persistence is simply skipped). <!-- Source: post-mortem, plush-press #4, 2026-06-09 -->
