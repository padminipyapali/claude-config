...
- **Content hash must include source identity, not just content.** When content-addressing chunks for dedup/upsert (ON CONFLICT on content_hash), the hash must include the source file/collection, not just the chunk text. Identical text appearing in two sources (e.g., same paragraph in PRODUCT_SPEC.md and DECISIONS.md) produces the same hash, causing one to silently overwrite the other. Pattern: `hash(sourceFile + "\n" + content)`. This applies to any content-addressed storage: embeddings, document chunks, cache entries, asset dedup. <!-- Source: PR review, second-brain #237, 2026-02-24 -->
