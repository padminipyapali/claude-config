...
- [ ] **Mutation → refresh-channel trace (fires when a PR adds or changes a mutation whose result is displayed OUTSIDE the mutating component).** For each mutation, enumerate every currently-mounted component that renders data derived from what the mutation changed, and name each one's refresh trigger. Trace **both directions independently** — "A mutates, B displays" and "B mutates, A displays" are different paths that fail separately. A component that stays mounted and does not subscribe to the shared invalidation signal is a defect, not an optimization; a local `reload()` that refreshes only the mutating component is the usual tell. This class is unreachable from the diff by construction: the bug is in what the code does NOT call, so it appears in no added line. Find it by tracing the refresh channel end-to-end, not by reading the changeset. <!-- Source: post-mortem, second-brain #948, 2026-07-27 — pinning never refreshed the Today card and unpinning left the TODOs panel rendering a lit aria-pressed="true" pin for an unpinned todo; both directions broken, neither visible in the diff -->
...
- [ ] **New union member completeness.** Grep entire codebase for every switch/map/conditional on that type. Check validation Sets — split shared constants per purpose when semantics diverge. Before adding a type to a Set that drives an *action/pending UI*, verify the type actually has a resolution path: its interceptor/callback must both **stamp** and **read** a resolution marker. A member whose interceptor stamps/reads no marker (e.g. read-only clarification flows) becomes false-pending forever and a "Clear" never sticks — exclude it or make the interceptor honor a marker. <!-- Source: post-mortem, second-brain #886, 2026-07-11 (CALENDAR_INQUIRY in AWAITING_PENDING_RESPONSE_TYPES) -->
...
- [ ] **Cached-async hook loading contract: `loading` must stay true until the FIRST resolution settles.** Any `use*` hook that resolves async state (fetch/resolve → cached value) must not report `loading: false` while the initial request is still in flight — a `null`/default data value with `loading: false` is indistinguishable from "resolved to nothing" and downstream gates will act on it. Require a `settled` (or equivalent) flag distinct from the cached value, plus a test asserting the initial `loading: true` state before the first fetch lands. This is the hook-contract shade of the cached/keyed-async-state family CodeRabbit has caught on 4 consecutive plush-press PRs (#330 partial-failure supersede, #333 two-snapshot read, #334 stale-key rebind, #338 `useResolvedStyle` loading-before-settled) after a critic SHIP each time. <!-- Source: post-mortem, plush-press #338, 2026-07-20 -->

- [ ] **A secondary fetch failure must NOT drop already-loaded data.** When an already-populated view fires a follow-up request — a post-action refetch, a "Load more" page, a background refresh — its error handler must NOT route into the top-level `error` state that replaces the whole view and blanks the loaded rows. Keep the last-good data + offer inline recovery (Retry / an inline banner), using a SEPARATE error state from the initial-load error. This class was caught twice in one series: second-brain #898 (a post-apply refetch failure wiped the health grid → keep counts + inline banner) and #903 (a failed "Load more" wiped the loaded message list → separate inline state + Retry). Check every component with more-than-one fetch: does a later fetch's catch clobber the earlier fetch's rendered data? <!-- Source: post-mortem, second-brain #898 + #903, 2026-07-13 -->
- [ ] **A spawned child / open stream holding a shared lock needs a bounded runtime + teardown.** When a route spawns a subprocess (`tar`, `pg_dump`, ffmpeg) or opens a long-lived stream while holding a single-in-flight concurrency guard/mutex, verify: (1) the child has a native spawn **timeout + SIGKILL** so a hang can't wedge the guard permanently; (2) a stream error handler (client abort → EPIPE) is handled locally, not bubbled to the process-level `uncaughtException` guard; (3) the guard is released and temp resources cleaned on ALL paths — success, error, AND client-abort. second-brain #900 (backup tar.gz stream): CodeRabbit caught an unbounded `tar` that could permanently wedge the one-at-a-time export guard, plus an EPIPE bubbling to the process guard — both robustness classes the adversarial pass initially missed. <!-- Source: post-mortem, second-brain #900, 2026-07-13 -->
- [ ] **Multi-step file mutation: a destructive first step needs restore-on-failure of every later step.** When an operation moves/renames/supersedes an existing file BEFORE writing its replacement (supersede-then-write, backup-then-overwrite, rename-then-create), a failure in any later step must restore the original (and all its siblings — sidecars, metadata) so the slot is never left empty or partial. Require: (1) try/catch around the post-destructive steps with an explicit restore path, and (2) a failure-injection test that makes the write throw and asserts the original is restored with no orphaned intermediate files. plush-press #330 (`saveSpecimen`): the fresh critic reviewed the supersede logic itself but missed that a write failure after the supersede rename left the specimen slot empty — CodeRabbit CLI caught it as its one MAJOR. <!-- Source: post-mortem, plush-press #330, 2026-07-19 -->
- [ ] **Route/handler test that MOCKS its service leaves the real query untested — require a sibling service-level test.** When a route/controller test mocks the service layer, the service's real SQL (parameter binding, user-scoping, cursor/limit, row→camelCase mapping, missing-relation/error contracts) is never exercised. Confirm a service-level test hits the real query for every method the route calls. second-brain #903: the route suite mocked `ProcessingLogService`, so the critic added 9 service-level tests for `listMessages`/`getMessage` covering scoping SQL, the exclusive cursor + LIMIT binding, mapping, and the 42P01 throw contract. This is the mock-boundary cousin of the falsely-green-gate concern. <!-- Source: post-mortem, second-brain #903, 2026-07-13 -->
...
- [ ] **Pattern siblings.** Grep entire codebase for same pattern. Same-file: fix now. Cross-module: file issue with `outside-diff` label.
  - **The sweep gets skipped precisely when the first instance was already FIXED.** Measured failure mode: reviewers run the sibling sweep when they're still hunting, but once a finding is written up and patched, the fix reads as closure and the grep never happens. second-brain #942 (2026-07-27): a "write succeeds but a conjunctive query filters the row out" bug was found and fixed on the pin route by human-style review; the IDENTICAL bug on the journal-seal route survived that pass and was caught only by CodeRabbit, because nobody grepped for other writers of the same conjunction after the first fix landed. Make the sweep a step of the FIX, not a step of the hunt: the moment you write a fix, grep for the pattern's other instances BEFORE marking the finding resolved, and record the grep command + every file it matched + each one's disposition in the evidence. A finding with no accompanying sibling-grep output is not finished. This is also the highest-value thing an automated reviewer contributes that a human pass reliably misses — treat a CodeRabbit finding that is a sibling of an already-fixed bug as a signal that the sweep was skipped, not as a bonus catch.
