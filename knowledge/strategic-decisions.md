...
- **For a relevance/noise threshold (similarity ceiling, score floor, k), don't ship a guessed default — calibrate it against a dry-run over the REAL corpus and read the output distribution before picking the number; for a "surface related items" feature, bias the chosen point toward *selective* because it fails by being noisy, not by being absent.** second-brain connection-graph shipped 0.65/0.08 defaults that, run over the actual 504-entry corpus, produced a hairball (375/504 connected, 2751 edges, 319 entries pinned at the out-degree cap — a degenerate flat distribution); recalibrating to 0.50/0.25 against the same dry-run gave 193 connected / 534 edges with a healthy *decaying* degree distribution (~8% at the cap). The tell that a threshold is mis-set isn't a single metric but the *shape*: a relatedness graph where a large fraction of nodes max out the cap is noise, not connection. Generalizes to any retrieval/ranking cutoff: the right value is an empirical property of your data, so measure the connected-fraction / edge-count / degree-distribution at a few candidate thresholds and pick the one whose distribution decays, accepting under-connection over a hairball. Document the chosen floor's side effects (here: a shared-tag-ONLY edge can't clear the 0.25 fused floor, so the graph is semantic-driven with tags enriching rather than connecting) and name the explicit lever to revisit as the corpus grows. Complements #747's "build for the row count you have" — same instinct (right-size to actual data) applied to a tuning constant rather than to feature scope.
<!-- Source: adversarial review, second-brain connection-graph tuning (#797), 2026-06-27 -->
